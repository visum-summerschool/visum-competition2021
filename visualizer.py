...
def triplet_outfit_visualizer(
    ...
    Args:
        sample (dict): sample of dataset
        inv_transform (transform): inverse transform applied to images
        word_map (dict): dictionary mapping from word to token id
        category_map (dict): dictionary mapping from category to id
...
def display_outfit_from_batch(
    batch, sample_index, word_map, category_map, save_fn=None
):
    """
    Display outfit info from batch

    Args:
        batch (dict): dictionary with a batch of data
...
def display_training_curves(training_data, trainning_labels, save_fn=None):
    """
...
def FITB_query_viz(config, df_queries, df_predictions, df_solutions, save_path=""):
    """
    ...
    Args:
        ...
        save_path (str): path to directory to save generated figures
    """
    ...
    for idx, (query, predictions, ground_truth) in enumerate(
        ...
        save_fn = os.path.join(save_path, str(idx) + "_{}.png")
...
def FITB_random_query_viz(
    ...
    Args:
        ...
        save_path (str): parent directory where plots will be saved
    """
    ...
    for idx, (query, predictions, ground_truth) in enumerate(
        ...
        save_fn = os.path.join(save_path, "correct_" + str(idx) + "_{}.png")
        display_fitb_query(query, predictions, ground_truth, images_dir, save_fn)
    ...
    for idx, (query, predictions, ground_truth) in enumerate(
        ...
        save_fn = os.path.join(save_path, "incorrect_" + str(idx) + "_{}.png")
        display_fitb_query(query, predictions, ground_truth, images_dir, save_fn)
